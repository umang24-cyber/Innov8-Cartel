...
import { ShieldCheck, Mail, Lock, Eye, EyeOff, ArrowRight, Sparkles, Activity, Brain, Layers, BarChart3, ChevronDown, Fingerprint, Search, FileText } from 'lucide-react';
...
export const LandingPage: React.FC<LandingPageProps> = ({ onSignUp, onLogin }) => {
            ...
            (entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        if (id === 'login') setLoginVisible(true);
                    }
                });
            },
            { threshold: 0.15 }
    ...
    return (
        <div className={`fixed inset-0 z-[9998] overflow-y-auto overflow-x-hidden transition-all duration-700 ${fadeOut ? 'opacity-0 scale-105' : 'opacity-100 scale-100'}`}
            style={{ scrollBehavior: 'smooth' }}
        >
            {/* Global dark background */}
            <div className="fixed inset-0 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 -z-20" />

            {/* Floating ambient orbs (fixed position, always visible) */}
            ...
            <section className="min-h-screen flex flex-col items-center justify-center relative px-6">
                <div className={`text-center max-w-4xl mx-auto transition-all duration-1000 ease-out ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
                    ...
                    <div className="flex items-center justify-center gap-4 mb-8">
                        <div className="relative">
                            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center shadow-2xl shadow-teal-500/30">
                    ...
                    <div className={`transition-all duration-1000 delay-700 ease-out ${heroVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                        <div className="flex items-center justify-center gap-3 mb-5">
                            ...
                            <span className="text-lg font-bold tracking-widest text-slate-300 uppercase">AI-Powered Fraud Intelligence</span>
                            <div className="h-px w-12 bg-teal-500/50" />
                        </div>
                ...
                <button
                    ...
                    className={`absolute bottom-10 flex flex-col items-center gap-2 text-slate-500 hover:text-teal-400 transition-all duration-700 delay-1000 cursor-pointer ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
                >
            ...
            {/* SECTION 2: ABOUT US                                        */}
            {/* ═══════════════════════════════════════════════════════════ */}
            <section ref={aboutRef} data-section="about" className="min-h-screen flex items-center justify-center px-6 py-24 relative">
                <div className={`max-w-5xl mx-auto transition-all duration-1000 ease-out ${aboutVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-16'}`}>
                    ...
                    <div className="text-center mb-16">
                        <span className="text-[11px] font-black text-teal-400 uppercase tracking-[0.4em] mb-4 block">Who We Are</span>
                        ...
                        {[
                            {
                                ...
                                desc: 'Isolation Forest algorithms identify statistical outliers in billing patterns that deviate from provider norms.',
                            },
                            {
                                icon: <Search className="w-7 h-7" />,
                                title: 'Explainable AI',
                                desc: 'Every flag comes with SHAP-powered explanations so investigators understand exactly why a claim was flagged.',
                            },
                            {
                                icon: <FileText className="w-7 h-7" />,
                                title: 'Clinical NLP Audit',
                                desc: 'LLM reads doctor\'s notes and cross-references them against billed procedures to catch semantic mismatches.',
                            },
                        ].map((card, i) => (
                            <div
                                key={i}
                                className={`relative group bg-white/[0.04] backdrop-blur-xl border border-white/[0.06] rounded-2xl p-6 transition-all duration-700 hover:border-teal-500/30 hover:bg-white/[0.06] hover:-translate-y-1 hover:shadow-xl hover:shadow-teal-500/5`}
            ...
            {/* ═══════════════════════════════════════════════════════════ */}
            <section ref={modelRef} data-section="model" className="min-h-screen flex items-center justify-center px-6 py-24 relative">
                <div className={`max-w-5xl mx-auto transition-all duration-1000 ease-out ${modelVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-16'}`}>
                    ...
                    <div className="text-center mb-16">
                        <span className="text-[11px] font-black text-cyan-400 uppercase tracking-[0.4em] mb-4 block">Under the Hood</span>
                        <h2 className="text-4xl md:text-5xl font-black text-white mb-6 tracking-tight">
                        ...
                        {[
                            {
                                ...
                                desc: 'Trained on historical claims data with SMOTE oversampling to handle class imbalance. Outputs a fraud probability between 0-100%. SHAP TreeExplainer provides per-feature attribution.',
                            ...
                            },
                            {
                                ...
                                desc: 'Unsupervised model that scores each claim on how "anomalous" it is relative to the historical distribution. Uses Total_Claim_Amount and Incident_Severity as key features.',
                            ...
                            },
                            {
                                ...
                                layer: 'Layer C',
                                title: 'LLM Clinical Audit (Groq + Mega)',
                                desc: 'Groq reads unstructured doctor\'s notes to check if clinical documentation justifies the billed procedures. Mega LLM powers the interactive investigator chatbot for deep-dive queries.',
                            ...
                            <div
                                key={i}
                                className={`relative flex gap-6 items-start bg-white/[0.04] backdrop-blur-xl border border-white/[0.06] rounded-2xl p-6 transition-all duration-700 hover:border-white/[0.12] hover:bg-white/[0.06] group`}
                            ...
                            >
                                ...
                                <div>
                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">{layer.layer}</span>
                                    <h3 className="text-white font-bold text-lg mb-1">{layer.title}</h3>
                    ...
                    <div className="mt-10 text-center">
                        <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 rounded-full px-5 py-2 text-amber-400 text-xs font-bold uppercase tracking-wider">
            ...
            {/* ═══════════════════════════════════════════════════════════ */}
            {/* SECTION 4: LOGIN / SIGN UP                                 */}
            {/* ═══════════════════════════════════════════════════════════ */}
            <section ref={loginRef} data-section="login" className="min-h-screen flex items-center justify-center px-6 py-24 relative">
                <div className={`w-full max-w-md mx-auto transition-all duration-1000 ease-out ${loginVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-16'}`}>

                    {/* Quote */}
                    <div className="text-center mb-10">
                        <span className="text-[11px] font-black text-teal-400 uppercase tracking-[0.4em] mb-4 block">Get Started</span>
                    ...
                    <div className="relative">
                        ...
                        <div className="relative bg-white/[0.07] backdrop-blur-2xl border border-white/[0.1] rounded-3xl p-8 shadow-2xl overflow-hidden">
                            ...
                            <p className="text-center text-[11px] text-slate-500 mt-4 font-medium">Protected by enterprise-grade encryption</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <footer className="py-8 text-center border-t border-white/[0.04]">
                <p className="text-slate-600 text-xs font-medium">© 2026 VeriClaim AI · Advisory Only · Innov8 Cartel</p>
            </footer>
        </div>
    );
};
